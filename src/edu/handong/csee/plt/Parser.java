...
public class Parser {
	...
	AST parse(String exampleCode) {
		...
		if(subExpressions.get(0).equals("fun")) {
			ArrayList<String> subExpressions2 = splitExpressionAsSubExpressions(subExpressions.get(1));
			return new Fun(subExpressions2.get(0),parse(subExpressions.get(2)));
		}
		
		if(subExpressions.get(0).equals("with")) {
			ArrayList<String> subExpressions2 = splitExpressionAsSubExpressions(subExpressions.get(1));
			return new App(
					new Fun(subExpressions2.get(0),parse(subExpressions.get(2))),
					parse(subExpressions2.get(1))
			);
		}
		
		if(subExpressions.size() == 2) {
			return new App(
					parse(subExpressions.get(0)),
					parse(subExpressions.get(1))
			);
		}
